...
import {
  ...
  Loader2, Save, X, BookOpen, ImageIcon, Download, Eye, FileImage
} from "lucide-react";
import Image from "next/image";
import Link from "next/link";
import AuthGuard from "@/components/auth/AuthGuard";
import { db, storage } from "@/lib/firebase";
import { collection, query, orderBy, onSnapshot, updateDoc, doc, setDoc, getDocs, where, deleteDoc, collectionGroup } from "firebase/firestore";
import { ref, uploadBytes, getDownloadURL } from "firebase/storage";

export default function AdminDashboard() {
  const { logout } = useAuth();
  const [pendingContracts, setPendingContracts] = useState<any[]>([]);
  const [todayContracts, setTodayContracts] = useState<any[]>([]);
  ...
  const [showQuoteToContractModal, setShowQuoteToContractModal] = useState(false);
  const [contractConversionData, setContractConversionData] = useState({
    eventDate: "",
    staffCount: 1,
    shiftType: "medio",
    contractTerm: "6",
  ...
  // --- REPORTES DIARIOS Y EXPORTACIÓN ---
  const [dailyReports, setDailyReports] = useState<Record<string, any>>({});
  const [showDailyReportViewer, setShowDailyReportViewer] = useState(false);
  const [activeDailyReport, setActiveDailyReport] = useState<any>(null);
  const [isExportingCSV, setIsExportingCSV] = useState(false);

  // --- HISTORIAL GLOBAL DE REPORTES ---
  const [showReportHistoryModal, setShowReportHistoryModal] = useState(false);
  const [allReportsHistory, setAllReportsHistory] = useState<any[]>([]);
  const [loadingReportsHistory, setLoadingReportsHistory] = useState(false);
  ...
  useEffect(() => {
    ...
    // Cargar contratos activos (que son eventos de hoy o contratos activos)
    // Primero, query genérico que traiga "contratos" o eventos
    const qA = query(collection(db, "contratos"), where("status", "==", "aprobado"));
    const unsubContracts = onSnapshot(qA, (snapshot) => {
      const today = new Date().toISOString().split('T')[0];
      const data = snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() }));

      // Filtrar solo los pertinentes a "hoy" o recurrentes
      const activeContracts = data.filter((c: any) => {
        if (c.serviceType === "contrato" && c.status === "aprobado") return true;
        if (c.serviceType !== "contrato" && c.eventDate === today && c.status === "aprobado") return true;
        return false;
      });

      setTodayContracts(activeContracts);
      setActiveContracts(data); // Keep all approved contracts for KPI
      setLoadingContracts(false);

      // --- NUEVO: Cargar reportes diarios para cada contrato activo ---
      activeContracts.forEach((contract: any) => {
        const reportsRef = collection(db, "contratos", contract.id, "reportes");
        const todayStart = new Date();
        todayStart.setHours(0, 0, 0, 0);

        const qReports = query(reportsRef, orderBy("timestamp", "desc"));
        onSnapshot(qReports, (repSnap) => {
          if (!repSnap.empty) {
            const latestReport: any = { id: repSnap.docs[0].id, ...repSnap.docs[0].data() };
            // Verificar si el reporte es de hoy
            const reportDate = latestReport.timestamp?.toDate ? latestReport.timestamp.toDate() : new Date();
            if (reportDate >= todayStart) {
              setDailyReports(prev => ({ ...prev, [contract.id]: latestReport }));
            }
          }
        });
      });
    ...
    const qPending = query(collection(db, "contratos"), where("status", "==", "solicitud"));
    const unsubPending = onSnapshot(qPending, (snapshot) => {
      setPendingContracts(snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() })));
    });

    const qAcceptedQuotes = query(collection(db, "cotizaciones"), where("status", "==", "aceptada_por_cliente"));
    const unsubAcceptedQuotes = onSnapshot(qAcceptedQuotes, (snapshot) => {
      setAcceptedQuotes(snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() })));
    });

    // Cargar historial global de reportes
    const unsubReportHistory = onSnapshot(query(collectionGroup(db, 'reportes'), orderBy('timestamp', 'desc')), async (snapshot) => {
      const reports = snapshot.docs.map(d => ({ id: d.id, ...d.data() })) as any[];
      setAllReportsHistory(reports);
    });
  ...
  const handleCreateQuoteFromRequest = (req: any) => {
    setNewQuoteData({
      userId: req.userId || "",
      clientNameFallback: req.personName || req.userEmail,
      serviceType: req.serviceType || "limpieza-recurrente",
      suppliesCost: 0,
      validityDays: "15",
      notes: "De acuerdo a su solicitud en el portal web, emitimos el siguiente presupuesto:"
    });
    setQuoteSearchClient(req.personName || req.userEmail);
    setQuoteConcepts([{ id: Date.now(), description: `Servicio Limpieza ${req.staffCount} pax en ${req.serviceAddress || 'domicilio'}`, qty: 1, unitPrice: 0 }]);
  ...
  const handleCreateQuotation = async () => {
    ...
    try {
      ...
      if (isEditingQuote && editingQuoteId) {
        await updateDoc(doc(db, "cotizaciones", editingQuoteId), quoteDoc);
        alert("Cotización actualizada con éxito.");
      } else {
        quoteDoc.status = "enviada";
        quoteDoc.createdAt = new Date().toISOString();
      ...
      console.error(err);
      alert(isEditingQuote ? "Fallo al actualizar la cotización" : "Fallo al crear la cotización");
    } finally {
  ...
  const handleConvertToContract = async () => {
    ...
    if (!contractConversionData.serviceAddress) return alert("La dirección del servicio es obligatoria.");
    setIsSavingQuote(true);
    try {
      const contractId = `CTR-${Date.now()}`;
      const contractDoc = {
        id: contractId,
        personName: activeQuote.clientName,
        userEmail: activeQuote.clientEmail,
        contactPhone: activeQuote.clientPhone,
        serviceType: activeQuote.serviceType,
        serviceAddress: contractConversionData.serviceAddress,
        eventDate: contractConversionData.eventDate,
        staffCount: contractConversionData.staffCount,
        shiftType: contractConversionData.shiftType,
        contractTerm: contractConversionData.contractTerm,
        needsSupplies: contractConversionData.needsSupplies,
        status: "aprobado",
        amount: activeQuote.amount,
        createdAt: new Date().toISOString(),
        isFromQuote: true,
        originalQuoteId: activeQuote.id
      };
      
      // Save contract
      await setDoc(doc(db, "contratos", contractId), contractDoc);
      
      // Update quote status
      await updateDoc(doc(db, "cotizaciones", activeQuote.id), { status: "aceptada" });
      
      alert("¡Contrato generado y cotización aceptada exitosamente!");
      setShowQuoteToContractModal(false);
      setShowQuoteViewer(false);
      
      // Reset
      setContractConversionData({ eventDate: "", staffCount: 1, shiftType: "medio", contractTerm: "6", serviceAddress: "", needsSupplies: false });
    } catch (err) {
      console.error(err);
      alert("Hubo un error al convertir a contrato.");
    } finally {
      setIsSavingQuote(false);
    }
  };
  ...
  const handleExportGlobalReport = async () => {
    ...
    try {
      // Obtener todos los contratos aprobados
      const snapshot = await getDocs(query(collection(db, "contratos"), where("status", "==", "aprobado")));

      let csvContent = "data:text/csv;charset=utf-8,";
      csvContent += "ID Servicio,Cliente,Email,Tipo Servicio,Fecha Inicio,Personal,Staff Asignado,RFC,Telefono\n";

      snapshot.docs.forEach(docSnap => {
        const data = docSnap.data();
        const staffName = data.assignedStaffUid ? usersList.find(u => u.id === data.assignedStaffUid)?.name || "Asignado (ID Local)" : "Sin Asignar";
        const clientName = data.personName || data.userEmail || "Desconocido";
        const row = [
          docSnap.id,
          `"${clientName}"`,
          data.userEmail || "",
      ...
      link.setAttribute("download", `Reporte_Global_LimpiezaMexico_${new Date().toISOString().split('T')[0]}.csv`);
  ...
  return (
    <AuthGuard requireAdmin={true}>
      <div className="py-8 print:hidden print:h-0 print:m-0 print:p-0">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-8">
                  ...
                  <table className="min-w-full divide-y divide-gray-200">
                    <thead>
                      <tr>
                        ...
                        <th className="px-6 py-4 text-left text-xs font-bold text-gray-500 uppercase tracking-wider bg-gray-50/50">Reporte Diario</th>
                      ...
                      {loadingContracts ? (
                        ...
                        <tr key={row.id} className="hover:bg-emerald-50/30 transition group cursor-pointer" onClick={() => handleOpenContractDetails(row)}>
                          ...
                          <td className="px-6 py-4" onClick={(e) => {
                            e.stopPropagation();
                            if (dailyReports[row.id]) {
                              setActiveDailyReport({ ...dailyReports[row.id], contractContext: row });
                              setShowDailyReportViewer(true);
                            }
                          }}>
                            {dailyReports[row.id] ? (
                              <button className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-bold bg-emerald-100 text-emerald-800 hover:bg-emerald-200 transition">
                                <Eye className="w-4 h-4" /> Entregado
                              </button>
                            ) : (
                              <span className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-bold bg-white border border-gray-200 text-gray-500">
            ...
            {/* Solicitudes y Cotizaciones Ganadas Section */}
            <div className="bg-white rounded-2xl shadow-sm border border-emerald-200 overflow-hidden">
              <div className="p-6 border-b border-emerald-100 flex justify-between items-center bg-emerald-50/50">
                <h2 className="text-lg font-bold text-gray-900 flex items-center gap-2"><ShieldAlert className="w-5 h-5 text-emerald-600" /> Acuerdos por Cerrar y Solicitudes</h2>
                <span className="bg-emerald-100 text-emerald-800 text-xs font-bold px-3 py-1 rounded-full">{pendingContracts.length + acceptedQuotes.length} Acciones</span>
              </div>
              <div className="p-6 space-y-4">
                {loadingContracts ? (
                  <div className="text-center py-8 text-gray-500 flex justify-center items-center gap-2">
                    <Loader2 className="w-6 h-6 animate-spin" /> Escaneando pipeline...
                  </div>
                ) : (pendingContracts.length === 0 && acceptedQuotes.length === 0) ? (
                  <div className="text-center py-8 text-emerald-600 font-medium bg-emerald-50/30 rounded-xl">
                    Pipeline comercial al día. Todas las solicitudes e instalaciones están cubiertas.
                  </div>
                ) : (
                  <>
                    {/* Cotizaciones Aceptadas por el Cliente */}
                    {acceptedQuotes.map((quote) => (
                      <div key={quote.id} className="border-2 border-emerald-200 bg-emerald-50/20 rounded-xl p-4 flex flex-col md:flex-row justify-between items-start md:items-center gap-4 transition shadow-sm">
                        <div className="flex flex-col">
                          <div className="flex items-center gap-2 mb-1">
                            <span className="bg-emerald-600 text-white text-[10px] font-black uppercase px-2 py-0.5 rounded flex items-center gap-1"><CheckCircle2 className="w-3 h-3"/> Aceptada</span>
                            <span className="text-xs font-bold text-gray-500 w-16 truncate" title={quote.id}>{quote.id.substring(0, 8)}</span>
                            <h4 className="font-bold text-gray-900">{quote.clientName}</h4>
                          </div>
                          <p className="text-sm text-gray-600 font-medium">
                            El cliente ha firmado digitalmente / aprobado formalmente su propuesta por <span className="font-bold text-emerald-700">${(quote.amount || 0).toLocaleString()} MXN</span>.
                          </p>
                        </div>
                        <div className="flex gap-2 w-full md:w-auto mt-4 md:mt-0">
                          <button onClick={() => { setActiveQuote(quote); setShowQuoteToContractModal(true); }} className="flex-1 md:flex-none px-6 py-2.5 bg-orange-500 hover:bg-orange-600 text-white text-sm font-black rounded-lg transition shadow-md whitespace-nowrap">
                            Formalizar Contrato Operativo
                          </button>
                        </div>
                      </div>
                    ))}

                    {/* Solicitudes de Servicio desde el Wizard */}
                    {pendingContracts.map((req) => (
                      <div key={req.id} className="border border-gray-200 rounded-xl p-4 flex flex-col md:flex-row justify-between items-start md:items-center gap-4 hover:border-blue-200 hover:bg-blue-50/10 transition">
                        <div>
                          <div className="flex items-center gap-2 mb-1">
                            <span className="bg-blue-100 text-blue-700 text-[10px] font-black uppercase px-2 py-0.5 rounded">Requerimiento Web</span>
                            <h4 className="font-bold text-gray-900">{req.personName || req.userEmail}</h4>
                          </div>
                          <p className="text-sm text-gray-600 mt-1">
                            <span className="font-medium mr-2">Servicio requerido:</span>
                        ...
                        <div className="flex gap-2 w-full md:w-auto mt-4 md:mt-0">
                          <button onClick={() => handleOpenContractDetails(req)} className="flex-1 md:flex-none px-4 py-2 border border-gray-300 text-sm font-semibold rounded-lg hover:bg-gray-50 transition">Ver Info Real</button>
                          <button onClick={() => handleCreateQuoteFromRequest(req)} className="flex-1 md:flex-none px-6 py-2 bg-gray-900 hover:bg-gray-800 text-white text-sm font-bold rounded-lg transition overflow-hidden text-ellipsis whitespace-nowrap">
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 flex items-center justify-between">
              ...
              <button onClick={handleExportGlobalReport} disabled={isExportingCSV} className="bg-gray-900 hover:bg-gray-800 text-white font-semibold py-2 px-6 rounded-lg shadow-sm transition-colors">
                {isExportingCSV ? <Loader2 className="w-4 h-4 animate-spin" /> : "Exportar CSV"}
              </button>
            </div>
          </div>
          ...
          <div className="space-y-6">
            <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
              ...
              <ul className="space-y-2">
                ...
                <li><button onClick={() => setShowReportHistoryModal(true)} className="w-full text-left px-4 py-3 rounded-lg hover:bg-emerald-50 font-medium text-emerald-700 transition flex justify-between items-center group border border-emerald-100"><span className="flex items-center gap-2"><FileImage className="w-4 h-4 text-emerald-500 group-hover:text-emerald-600" /> Historial de Reportes</span> <ChevronRight className="w-4 h-4 text-emerald-400 group-hover:text-emerald-600" /></button></li>
              </ul>
            </div>
          </div>
      ...
      {/* --- MODAL VISOR DE REPORTE DIARIO --- */}
      {showDailyReportViewer && activeDailyReport && (
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl max-w-lg w-full overflow-hidden shadow-2xl relative animate-in fade-in zoom-in-95">

            <div className="p-6 border-b border-gray-100 bg-gray-50 flex justify-between items-start">
              <div>
                <h2 className="text-xl font-bold text-gray-900 flex items-center gap-2"><FileImage className="w-5 h-5 text-emerald-600" /> Reporte de Supervisión</h2>
                <p className="text-sm text-gray-500 mt-1">
                  {activeDailyReport.contractContext?.personName || "Cliente"} • {new Date(activeDailyReport.timestamp?.seconds * 1000).toLocaleString() || "Hoy"}
                </p>
              </div>
              <button onClick={() => { setShowDailyReportViewer(false); setActiveDailyReport(null); }} className="text-gray-400 hover:text-gray-600 bg-gray-200 rounded-full p-1.5 focus:outline-none focus:ring-2 focus:ring-emerald-500">
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            <div className="p-6 space-y-6">
              <div>
                <h3 className="text-xs font-bold text-gray-500 uppercase tracking-wider mb-2">Comentarios del Personal</h3>
                <div className="bg-gray-50 p-4 rounded-xl text-sm text-gray-800 border border-gray-100 whitespace-pre-wrap leading-relaxed">
                  {activeDailyReport.text || "Sin comentarios."}
                </div>
              </div>

              {activeDailyReport.imageUrl ? (
                <div>
                  <h3 className="text-xs font-bold text-gray-500 uppercase tracking-wider mb-2">Evidencia Fotográfica</h3>
                  <div className="relative h-64 w-full rounded-xl overflow-hidden shadow-sm border border-gray-200 bg-gray-100 group">
                    <Image src={activeDailyReport.imageUrl} alt="Evidencia supervisión" fill className="object-cover" unoptimized />
                    <a href={activeDailyReport.imageUrl} target="_blank" rel="noopener noreferrer" className="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition flex items-center justify-center text-white font-bold gap-2">
                      <Eye className="w-5 h-5" /> Ver Imagen Completa
                    </a>
                  </div>
                </div>
              ) : (
                <div className="bg-orange-50 p-4 rounded-xl flex items-center gap-3 text-orange-800 border border-orange-100">
                  <ShieldAlert className="w-5 h-5 text-orange-500" />
                  <span className="text-sm font-medium">No se adjuntó evidencia fotográfica en este reporte.</span>
                </div>
              )}
            </div>
          </div>
      ...
      {/* --- MODAL DE CONVERSIÓN A CONTRATO (FASE 24) --- */}
      {showQuoteToContractModal && (
        <div className="fixed inset-0 bg-black/60 z-[70] flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl max-w-lg w-full p-6 sm:p-8 shadow-2xl relative animate-in fade-in zoom-in-95">
            <button onClick={() => setShowQuoteToContractModal(false)} className="absolute top-4 right-4 text-gray-400 hover:text-gray-600 bg-gray-100 p-2 rounded-full">
              <X className="w-5 h-5" />
            </button>
            <h2 className="text-xl font-black text-gray-900 mb-2">Convertir a Contrato Operativo</h2>
            <p className="text-sm text-gray-500 mb-6">Complete los requisitos operativos para formalizar el contrato de {activeQuote?.clientName}.</p>

            <div className="space-y-4">
              <div>
                <label className="text-xs font-bold text-gray-700 mb-1 block">Dirección o Sede del Servicio <span className="text-red-500">*</span></label>
                <input type="text" value={contractConversionData.serviceAddress} onChange={e => setContractConversionData({ ...contractConversionData, serviceAddress: e.target.value })} className="w-full p-2.5 border rounded-lg focus:ring-2 focus:ring-emerald-500 text-sm" placeholder="Ej. Av. Reforma 222..." />
              </div>

              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="text-xs font-bold text-gray-700 mb-1 block">Tipo de Turno</label>
                  <select value={contractConversionData.shiftType} onChange={e => setContractConversionData({ ...contractConversionData, shiftType: e.target.value })} className="w-full p-2.5 border rounded-lg focus:ring-2 focus:ring-emerald-500 bg-white text-sm">
                    <option value="medio">Medio Día (4hrs)</option>
                    <option value="completo">Completo (8hrs)</option>
                  </select>
                </div>
                <div>
                  <label className="text-xs font-bold text-gray-700 mb-1 block">Personal Requerido (Pax)</label>
                  <input type="number" min="1" value={contractConversionData.staffCount} onChange={e => setContractConversionData({ ...contractConversionData, staffCount: Number(e.target.value) })} className="w-full p-2.5 border rounded-lg focus:ring-2 focus:ring-emerald-500 text-sm" />
                </div>
              </div>

              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="text-xs font-bold text-gray-700 mb-1 block">Fecha de Inicio / Evento</label>
                  <input type="date" value={contractConversionData.eventDate} onChange={e => setContractConversionData({ ...contractConversionData, eventDate: e.target.value })} className="w-full p-2.5 border rounded-lg focus:ring-2 focus:ring-emerald-500 text-sm xl:text-xs" />
                </div>
                <div>
                  <label className="text-xs font-bold text-gray-700 mb-1 block">Plazo Forzoso (Meses)</label>
                  <input type="number" value={contractConversionData.contractTerm} onChange={e => setContractConversionData({ ...contractConversionData, contractTerm: e.target.value })} className="w-full p-2.5 border rounded-lg focus:ring-2 focus:ring-emerald-500 text-sm" />
                </div>
              </div>

              <div className="flex items-center gap-3 pt-2">
                <input type="checkbox" id="convNeedsSupplies" checked={contractConversionData.needsSupplies} onChange={e => setContractConversionData({ ...contractConversionData, needsSupplies: e.target.checked })} className="w-4 h-4 text-emerald-600 rounded focus:ring-emerald-500" />
                <label htmlFor="convNeedsSupplies" className="text-sm font-bold text-gray-700 cursor-pointer">El contrato incluye suma extra para insumos o consumibles.</label>
              </div>
            </div>

            <div className="mt-8 pt-4 border-t border-gray-100 flex justify-end gap-3">
              <button onClick={() => setShowQuoteToContractModal(false)} className="px-5 py-2.5 text-gray-600 font-bold hover:bg-gray-100 rounded-lg transition">Cancelar</button>
              <button onClick={handleConvertToContract} disabled={isSavingQuote} className="flex items-center gap-2 px-6 py-2.5 bg-orange-500 text-white font-bold rounded-lg hover:bg-orange-600 disabled:opacity-50 transition shadow-md">
                {isSavingQuote ? <Loader2 className="w-5 h-5 animate-spin" /> : "Aprobar y Generar Contrato"}
              </button>
            </div>
          </div>
        </div>
      ...
      {/* --- MODAL HISTORIAL DE REPORTES GLOBALES --- */}
      {showReportHistoryModal && (
        <div className="fixed inset-0 bg-black/50 z-50 flex justify-end">
          <div className="bg-white max-w-2xl w-full p-6 shadow-xl relative animate-in slide-in-from-right duration-300 flex flex-col h-full ml-auto overflow-hidden">
            <button onClick={() => setShowReportHistoryModal(false)} className="absolute top-4 right-4 text-gray-400 hover:text-gray-600 bg-gray-100 hover:bg-gray-200 p-2 rounded-full transition z-10">
              <X className="w-5 h-5" />
            </button>

            <div className="mb-6 pr-8 border-b border-gray-100 pb-4">
              <h2 className="text-xl font-bold text-gray-900 mb-2 flex items-center gap-2"><FileImage className="w-6 h-6 text-emerald-600" /> Historial de Reportes</h2>
              <p className="text-sm text-gray-500">Registro histórico de todos los reportes de supervisión completados por parte de los operarios.</p>
            </div>

            <div className="flex-grow overflow-auto border border-gray-100 rounded-xl relative shadow-sm bg-gray-50/50 p-4 space-y-4">
              {allReportsHistory.length === 0 ? (
                <div className="text-center py-10 text-gray-400 font-medium">No se han generado reportes operativos aún.</div>
              ) : allReportsHistory.map((report) => {
                return (
                  <div key={report.id} onClick={() => { setActiveDailyReport({ ...report }); setShowDailyReportViewer(true); }} className="bg-white p-4 rounded-xl shadow-sm border border-gray-100 hover:border-emerald-300 transition cursor-pointer flex gap-4">
                    <div className="flex-none">
                      {report.imageUrl ? (
                        <div className="w-16 h-16 rounded-lg bg-gray-100 relative overflow-hidden border border-gray-200">
                          <Image src={report.imageUrl} alt="Evidencia" fill className="object-cover" unoptimized />
                        </div>
                      ) : (
                        <div className="w-16 h-16 rounded-lg bg-orange-50 border border-orange-100 flex items-center justify-center text-orange-400">
                    ...
                    <div className="flex-grow min-w-0">
                      <div className="flex justify-between items-start mb-1">
                        <h4 className="font-bold text-gray-900 truncate">Reporte Diaro</h4>
                        <span className="text-xs font-semibold text-gray-500 bg-gray-100 px-2 py-0.5 rounded-full whitespace-nowrap">
                          {report.timestamp?.seconds ? new Date(report.timestamp.seconds * 1000).toLocaleDateString() : 'Reciente'}
                        </span>
                      </div>
                      <p className="text-sm text-gray-600 line-clamp-2 leading-snug">{report.text || "Sin comentarios anexos al reporte fotográfico."}</p>
                    </div>
                        ...
                        <td className="px-6 py-4">
                          <div className="flex items-center gap-2">
                            <select
                              ...
                              className={`px-3 py-1.5 rounded-lg text-xs font-bold outline-none border transition-all ${usr.role === 'admin' ? 'bg-emerald-50 text-emerald-800 border-emerald-200 shadow-sm' : 'bg-white text-gray-600 border-gray-200 hover:border-gray-300 shadow-sm'}`}
                            >
          ...
          <div className="bg-white max-w-2xl w-full p-6 sm:p-8 shadow-2xl relative animate-in slide-in-from-right duration-300 h-full overflow-y-auto flex flex-col ml-auto">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 mb-8">
              ...
              <div className="bg-gray-50 p-4 rounded-xl shadow-sm border border-gray-100">
                ...
                <p className="text-sm text-gray-800 font-medium">📍 {activeContractInfo.serviceAddress || 'Pendiente'}</p>
                    ...
                    <tbody className="divide-y divide-gray-100">
                      {allContractsHistory.map(row => (
                        <tr key={row.id} className="hover:bg-emerald-50/40 transition">
                          ...
                          <td className="px-6 py-4">
                            <div className="font-semibold text-gray-800 text-sm w-32 text-ellipsis overflow-hidden" title={row.personName || row.userEmail}>{row.personName || row.userEmail} <span className="text-gray-500 text-xs">{row.serviceAddress}</span></div>
                          </td>
      ...
      {/* --- CREADOR DE COTIZACIONES AVANZADO (FASE 22 / EDIT FASE 24) --- */}
      {showNewQuotation && (
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl max-w-3xl w-full p-6 sm:p-8 shadow-2xl relative animate-in fade-in zoom-in-95 max-h-[90vh] overflow-y-auto">
            ...
            <h2 className="text-2xl font-black text-gray-900 mb-6 flex items-center gap-2"><FileText className="w-6 h-6 text-emerald-600" /> {isEditingQuote ? "Editar Cotización" : "Creador de Cotización Empresarial"}</h2>
            ...
            <div className="mt-8 pt-5 border-t border-gray-200 flex flex-col sm:flex-row justify-between items-center gap-6 bg-gray-50 px-6 py-4 rounded-xl shadow-inner">
              ...
              <div className="flex gap-3 w-full sm:w-auto">
                <button onClick={() => {
                   ...
                   setNewQuoteData({ userId: "", clientNameFallback: "", serviceType: "limpieza-recurrente", suppliesCost: 0, validityDays: "15", notes: "" });
                   setQuoteConcepts([{ id: Date.now(), description: "", qty: 1, unitPrice: 0 }]);
                }} className="flex-1 sm:flex-none px-5 py-3 text-gray-600 font-bold hover:bg-gray-100 rounded-lg transition">Cancelar</button>
                <button onClick={handleCreateQuotation} disabled={isSavingQuote} className="flex-1 sm:flex-none flex items-center justify-center gap-2 px-8 py-3 bg-emerald-600 border border-emerald-700 text-white font-bold rounded-xl hover:bg-emerald-700 disabled:opacity-50 transition shadow-xl hover:shadow-emerald-900/20">
                  {isSavingQuote ? <Loader2 className="w-5 h-5 animate-spin" /> : <Save className="w-5 h-5" />} {isEditingQuote ? "Actualizar Cotización" : "Generar y Guardar"}
                </button>
              </div>
            </div>
          </div>
      ...
      {showQuoteViewer && activeQuote && (
        <>
          ...
          <div className="fixed inset-0 bg-gray-900/90 z-[60] flex flex-col items-center justify-start p-4 sm:p-8 overflow-y-auto print:static print:p-0 print:m-0 print:block print:overflow-visible">
            ...
            <div className="bg-white/10 backdrop-blur-md border border-white/20 p-4 rounded-2xl flex flex-wrap gap-4 justify-between items-center max-w-5xl w-full mb-6 shadow-2xl print:hidden sticky top-0 z-10">
              <div className="text-white hidden md:block">
                <h3 className="font-bold text-lg flex items-center gap-2"><FileText className="w-5 h-5" /> Pre-visualización</h3>
              </div>
              <div className="flex gap-2 w-full md:w-auto overflow-x-auto pb-2 md:pb-0 hide-scrollbar">
                <button onClick={() => {
                   setIsEditingQuote(true);
                   setEditingQuoteId(activeQuote.id);
                   setNewQuoteData({
                     userId: activeQuote.userId || "",
                     clientNameFallback: activeQuote.clientNameFallback || activeQuote.clientName,
                     serviceType: activeQuote.serviceType || "limpieza-recurrente",
                     suppliesCost: activeQuote.suppliesCost || 0,
                     validityDays: activeQuote.validityDays || "15",
                     notes: activeQuote.notes || ""
                   });
                   setQuoteSearchClient(activeQuote.clientName);
                   setQuoteConcepts(activeQuote.concepts || [{ id: Date.now(), description: "", qty: 1, unitPrice: 0 }]);
                   setShowQuoteViewer(false);
                   setShowNewQuotation(true);
                }} className="bg-blue-600 hover:bg-blue-500 text-white px-4 py-2 rounded-xl text-sm font-bold transition flex items-center gap-2 whitespace-nowrap">✎ Editar</button>
                
                <button onClick={() => setShowQuoteToContractModal(true)} className="bg-orange-500 hover:bg-orange-400 text-white px-4 py-2 rounded-xl text-sm font-bold transition flex items-center gap-2 whitespace-nowrap">Convertir a Contrato</button>
                
                <button onClick={() => setShowQuoteViewer(false)} className="bg-gray-800 hover:bg-gray-700 text-white px-4 py-2 rounded-xl text-sm font-bold transition flex items-center gap-2 whitespace-nowrap"><X className="w-4 h-4" /> Cerrar</button>
                
                <button onClick={handlePrint} className="bg-emerald-500 hover:bg-emerald-400 text-black px-6 py-2 rounded-xl text-sm font-black transition flex items-center gap-2 shadow-[0_0_15px_rgba(16,185,129,0.5)] whitespace-nowrap"><Save className="w-4 h-4" /> PDF</button>
              </div>
            </div>
